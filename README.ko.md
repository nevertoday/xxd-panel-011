...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-철학적-선-논리)
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090090613073232083"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 011 예시 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090090613073232083"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 011 예시 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090090613073232083"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 011 예시 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2090090613073232083"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 011 예시 4"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 011 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 011 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 011 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 011 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 011의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 011 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 011 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 011 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 011 추가 상하 예시 4"></td>
  </tr>
</table>

> 추가 예시: 천샹 포스터 그룹에서 해당 천샹 포스터 그룹의 네 장을 파일명 순서로 사용하고, 이 Panel의 원본 브리프에 따라 16:9 좌우 구성과 영어 자동 생성 문구로 제작했습니다.

> [샤오샤오둥（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2090090613073232083) · 2026-08-19<br>
> GPT2 x 线条 x 灵魂 x 极简 x 抽象 x 美学提示词<br>
> 원문 게시물에는 VOL 표기가 없지만, 작성자가 이번 대화에서 XXD Panel 011 예시임을 확인했습니다.

<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2090090613073232083">원문 게시물과 전체 프롬프트 보기 →</a></p>

이 예시는 011의 미학적 의도를 보여 줄 뿐이며, 예시의 주제, 구성, 색상, 문구, 이전 캔버스 비율은 생성 참고나 현재 기본값이 되지 않습니다.

> 하나의 핵심 이미지 · 하나의 관계 · 연속 검은 선 · 능동적 침묵 · 하나의 기억색

XXD Panel 011는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 정체성, 윤곽, 자세와 핵심 관계를 보존한 뒤 사진의 감정적 긴장, 공간 관계, 동작 방향, 거리, 상태 또는 근거 있는 은유를 하나의 시각 명제로 압축하고, 소수의 가늘고 흐르며 자연스러운 검정 연속선으로 재구성합니다.

멈춤, 호흡, 밀도, 어긋남, 연장, 겹침, 교차와 미완성이 서사를 맡습니다. 흰색 또는 따뜻한 종이 여백은 시간, 거리, 외로움, 친밀함, 기다림과 흐름을 표현합니다. 원본에서 가져온 아주 작은 색 하나는 남겨진 기억처럼 작동하고, 문자는 선과 함께 구성되는 아트북 주석이 됩니다.
...
**Panel 011**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 천천히 바라볼 수 있는 철학적 미니멀 연속선으로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 011으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 011이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 천천히 바라볼 수 있는 철학적 미니멀 연속선으로 |
| **한눈에 알아볼 특징** | 하나의 핵심 이미지 · 하나의 관계 · 연속 검은 선 · 능동적 침묵 · 하나의 기억색 |
...
일반적인 ‘미니멀 선화’는 예쁘지만 비어 있는 장식 곡선, 기계적인 한 줄 트레이싱 또는 어떤 사진에도 붙일 수 있는 철학 기호 세트로 쉽게 무너집니다.

011은 순서를 뒤집습니다.

```text
원본 정체성과 관계 고정 → 가장 의미 있는 감정／공간／방향／거리 명제 선택 → 군더더기를 지워 하나의 핵심 이미지로 → 멈춤, 거리, 연결, 어긋남, 연장, 겹침, 미완성으로 연속선 구성 → 침묵이 시간을 담당 → 기억색 하나 보존 → 문자를 선 구조에 통합
```

무관한 사진으로 바꿔도 핵심 이미지, 관계 거리, 선 경로, 멈춤, 교차, 기억색과 문구가 거의 그대로 성립한다면 011이 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **하나의 핵심 이미지:** 최소 세 가지 원본 고유 단서로 정체성, 윤곽, 자세, 동작, 구조와 관계를 보존합니다.
- **하나의 정신적 명제:** 가장 의미 있는 감정적 긴장, 공간 관계, 동작 방향, 거리, 상태 또는 근거 있는 은유만 추출합니다.
- **가는 연속 검은 선:** 자연스럽게 흐르는 선이 멈춤, 호흡, 밀도, 마찰, 망설임, 리듬과 미완성을 가집니다.
- **모든 변화에 근거:** 생략, 연결, 어긋남, 연장, 겹침, 교차와 열린 끝은 모두 원본에서 나옵니다.
- **여백이 서술:** 흰색 또는 따뜻한 종이가 지배하고 주제는 작고 비껴가거나 잘리고 떠 있을 수 있으며 침묵이 시간과 거리를 맡습니다.
- **기억색 하나:** 원본에서 가져온 아주 작은 색을 사물, 절점, 선분, 빛점 또는 관계 초점 한 곳에만 둡니다.
- **아트북 미세 문자:** 짧은 제목과 1–2개 미세 요소를 선을 따라, 선 끝에, 여백에, 윤곽에 또는 선의 기준선 위에 통합합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 011의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-011 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-011 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-011 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-011 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-011 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-011 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 011 적용, 공통 설정은 한 번만 확정
/xxd-panel-011 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-011`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-011-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-011-prompt.en.md)
...
**사진을 선화로 만들지 말고, 한 선에 시간과 거리와 끝나지 않은 관계를 남기세요.**
